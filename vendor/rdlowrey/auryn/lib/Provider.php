...
class Provider implements Injector
{
    ...
    public function alias($typehintToReplace, $alias)
    {
        if (empty($typehintToReplace) || !is_string($typehintToReplace)) {
            ...
            $this->errorMessages[self::E_NON_EMPTY_STRING_ALIAS], self::E_NON_EMPTY_STRING_ALIAS
            );
        ...
        if (isset($this->sharedClasses[$normalizedTypehint])) {
            ...
            sprintf($this->errorMessages[self::E_SHARED_CANNOT_ALIAS], $sharedClassName, $alias), self::E_SHARED_CANNOT_ALIAS
            );
        } else {
    ...
    public function share($classNameOrInstance)
    {
        if (is_string($classNameOrInstance)) {
            ...
            throw new BadArgumentException(
            sprintf($this->errorMessages[self::E_SHARE_ARGUMENT], __CLASS__, gettype($classNameOrInstance)), self::E_SHARE_ARGUMENT
            );
        }
    ...
    public function delegate($className, $callable, array $args = array())
    {
        if ($this->canExecute($callable)) {
            ...
            throw new BadArgumentException(
            sprintf($this->errorMessages[self::E_DELEGATE_ARGUMENT], __CLASS__), self::E_DELEGATE_ARGUMENT
            );
        }
    ...
    public function prepare($classInterfaceOrTraitName, $executable)
    {
        if (!$this->canExecute($executable)) {
            throw new BadArgumentException(
            $this->errorMessages[self::E_CALLABLE], self::E_CALLABLE
            );
        }
    ...
    public function buildExecutable($callableOrMethodArr, $makeAccessible = FALSE)
    {
        ...
        if ($makeAccessible && $reflectionFunction instanceof \ReflectionMethod && !$reflectionFunction->isPublic()
        ) {
    ...
    protected function getInjectedInstance($className, array $definition)
    {
        try {
            ...
            if (!$ctorMethod) {
                ...
                throw new InjectionException(
                sprintf($this->errorMessages[self::E_NON_PUBLIC_CONSTRUCTOR], $className), self::E_NON_PUBLIC_CONSTRUCTOR
                );
            ...
            throw new InjectionException(
            sprintf($this->errorMessages[self::E_MAKE_FAILURE], $className, $e->getMessage()), self::E_MAKE_FAILURE, $e
            );
        }
    }
    ...
    private function validateInjectionDefinition(array $injectionDefinition)
    {
        foreach ($injectionDefinition as $paramName => $value) {
            if ($paramName[0] !== self::RAW_INJECTION_PREFIX && !is_string($value)) {
                throw new BadArgumentException(
                sprintf($this->errorMessages[self::E_RAW_PREFIX], $paramName, $paramName), self::E_RAW_PREFIX
                );
            }
        }
    }
    ...
    private function generateExecutableReflection($exeCallable)
    {
        ...
        } elseif (is_array($exeCallable) && isset($exeCallable[0], $exeCallable[1]) && count($exeCallable) === 2
        ) {
    ...
    private function guardAgainstCyclicDependency($className, $normalizedClass)
    {
        if (isset($this->beingProvisioned[$normalizedClass])) {
            ...
            $className, sprintf($this->errorMessages[self::E_CYCLIC_DEPENDENCY], $className), self::E_CYCLIC_DEPENDENCY
            );
        }
    ...
    private function provisionFromDelegate($className)
    {
        ...
        if (!$provisionedObject instanceof $className) {
            throw new InjectionException(
            sprintf($this->errorMessages[self::E_INVALID_CLASS], $className, get_class($provisionedObject)), self::E_INVALID_CLASS
            );
        }
    ...
    private function provisionInstance($className, array $customDefinition)
    {
        ...
        try {
            ...
            throw new CyclicDependencyException(
            $cycleDetector, sprintf($this->errorMessages[self::E_CYCLIC_DEPENDENCY], $className), self::E_CYCLIC_DEPENDENCY, $e
            );
        }
    }
    ...
    private function selectParentDefinition($className, $childDefinition)
    {
        try {
            ...
            return $parent ? $this->selectParentDefinition($parent->getName(), $childDefinition) : array_merge($this->getDefinition($className), $childDefinition);
        } catch (\ReflectionException $e) {
            throw new InjectionException(
            sprintf($this->errorMessages[self::E_CLASS_NOT_FOUND], $className), self::E_CLASS_NOT_FOUND, $e
            );
        }
    }
    ...
    private function getDefinition($className)
    {
        ...
        return $this->isDefined($normalizedClass) ? $this->injectionDefinitions[$normalizedClass] : array();
    }
    ...
    private function generateStringClassMethodCallable($class, $method)
    {
        ...
        return $reflectionMethod->isStatic() ? array($reflectionMethod, NULL) : array($reflectionMethod, $this->make($class));
    }
    ...
    private function buildArgumentFromReflectionParameter(\ReflectionParameter $param)
    {
        if (array_key_exists($param->name, $this->paramDefinitions)) {
            ...
            throw new InjectionException(
            sprintf($this->errorMessages[self::E_UNDEFINED_PARAM], $declaringClass, $param->name), self::E_UNDEFINED_PARAM
            );
        }
    ...
    private function buildWithoutConstructorParams($className)
    {
        if ($this->isInstantiable($className)) {
            ...
            throw new InjectionException(
            sprintf($this->errorMessages[self::E_NEEDS_DEFINITION], $type, $className), self::E_NEEDS_DEFINITION
            );
        }
    ...
    private function shareClass($classNameOrInstance)
    {
        ...
        $this->sharedClasses[$normalizedClass] = isset($this->sharedClasses[$normalizedClass]) ? $this->sharedClasses[$normalizedClass] : NULL;
    }
    ...
    private function shareObject($classNameOrInstance)
    {
        ...
        if (isset($this->aliases[$normalizedClass])) {
            ...
            throw new InjectionException(
            sprintf($this->errorMessages[self::E_ALIASED_CANNOT_SHARE], $normalizedClass, $this->aliases[$normalizedClass]), self::E_ALIASED_CANNOT_SHARE
            );
        }
